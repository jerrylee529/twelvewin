...
"""Industry list and cluster payloads."""

from sqlalchemy.orm import Session

from api.db.models import Instrument, StockCluster, StockClusterItem
...
def list_industries(session: Session) -> list[dict]:
    industries = session.query(Instrument.industry).group_by(Instrument.industry).all()
    items = []
    index = 1
    for industry in industries:
        ...
        if not name:
            continue
        items.append({'id': index, 'name': name})
        index += 1
    return items


def get_industry_cluster_payload(session: Session, industry: str) -> dict:
    if not industry:
        return {'total': 0, 'rows': []}
    ...
    centers = session.query(StockCluster).filter(StockCluster.section == industry).all()
    center_labels = {center.code: index for index, center in enumerate(centers)}

    items = session.query(StockClusterItem).filter(StockClusterItem.section == industry).all()
    code_to_label = {}
    for item in items:
...
def get_industry_stock_payload(session: Session, industry: str, code: str) -> dict:
    payload = get_industry_cluster_payload(session, industry)
    ...
    for key in (
        'avg_pe', 'avg_pb', 'avg_esp', 'avg_bvps', 'avg_gpr', 'avg_npr',
